...
var backward_run_speed_multiplier : float = 0.7
...
func applies(delta : float) -> void:
	...
	if play_char.is_on_floor():
		if auto_bunny_hop and play_char.hit_ground_cooldown > 0.0 and play_char.input_direction != Vector2.ZERO and play_char.jump_cooldown <= 0.0:
...
func input_management() -> void:
	...
	if Input.is_action_just_pressed(play_char.crouch_action) and !priority_over_crouch:
...
func move(delta : float) -> void:
	...
	if play_char.move_direction and play_char.is_on_floor():
		new_velocity_x = lerp(
			...
			play_char.move_direction.x * speed * (backward_run_speed_multiplier if play_char.input_direction.y > 0.0 else 1.0), 
			acceleration * play_char.movement_acceleration_multiplier * delta)
			
		new_velocity_z = lerp(
			play_char.velocity.z, 
			play_char.move_direction.z * speed * (backward_run_speed_multiplier if play_char.input_direction.y > 0.0 else 1.0), 
			acceleration * play_char.movement_acceleration_multiplier * delta)
