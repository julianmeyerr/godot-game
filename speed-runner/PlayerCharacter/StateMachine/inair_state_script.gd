...
func move(delta : float) -> void:
	...
	if !play_char.is_on_floor():
		if play_char.move_direction:
			...
			#var contrd_des_move_speed : float = desired_move_speed_curve.sample(play_char.desired_move_speed)
			#var contrd_in_air_move_speed : float = in_air_move_speed_curve.sample(play_char.desired_move_speed) * 1.0
			...
			if play_char.walljump_lock_in_air_movement_time <= 0.0:
				...
				#new_velocity_x = lerp(play_char.velocity.x, play_char.move_direction.x * contrd_des_move_speed, contrd_in_air_move_speed * delta)
				#new_velocity_z = lerp(play_char.velocity.z, play_char.move_direction.z * contrd_des_move_speed, contrd_in_air_move_speed * delta)
