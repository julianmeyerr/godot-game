...
var fly_accel: float = 15.0
var fly_deccel: float = 15.0
...
func move(delta : float) -> void:
	...
	if play_char.move_direction:
		new_velocity = lerp(play_char.velocity, play_char.move_direction * fly_speed, fly_accel * delta)
	else:
		new_velocity = lerp(play_char.velocity, play_char.move_direction * fly_speed, fly_deccel * delta)
		
	play_char.set_horizontal(new_velocity)
	play_char.set_vertical(new_velocity.y)
