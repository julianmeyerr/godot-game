...
func input_management():
	...
	if Input.is_action_just_pressed(play_char.move_forward_action):
...
func move(delta : float):
	...
	play_char.desired_move_speed = clamp(play_char.desired_move_speed, 0.0, max_desired_move_speed)
	...
	if play_char.move_direction and play_char.is_on_floor():
		...
		#apply smooth stop 
		var deceleration: float = idle_deccel * play_char.movement_deceleration_multiplier
