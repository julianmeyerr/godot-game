...
@export var mouse_mode_action : StringName = "play_char_mouse_mode"
...
func _ready() -> void:
	...
	camera.fov = fov
	if state_machine.has_signal("change_fov"):
		state_machine.connect("change_fov", Callable(self, "change_fov"))
...
func _process(delta : float) -> void:
	...
	move_speed = Vector2(play_char.velocity.x, play_char.velocity.z).length()
	tilt(delta)
	...
	if state != "Fly" and state != "Slide" and state != "Wallrun":
		if enable_forward_tilt:
			...
			if has_started_moving_forward or has_started_moving_backward:
				...
				var tilt_offset : float = clamp((-play_char.input_direction.y * move_speed) / forward_move_tilt_divider, -forward_move_max_tilt_val, forward_move_max_tilt_val)
		...
		if enable_side_tilt:
			...
			clamp((-play_char.input_direction.x * move_speed) / side_move_tilt_divider, -side_move_max_tilt_val, side_move_max_tilt_val), 
